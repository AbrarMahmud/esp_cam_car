...
int left_track_pwm = 0;        //FOR controlling the vehicle
int right_track_pwm = 0;
...
static esp_err_t cmd_handler(httpd_req_t *req)
{
    ...
    else if (!strcmp(variable, "left_track"))
    {
      left_track_pwm=val;
    }
    else if (!strcmp(variable, "right_track"))
    {
      right_track_pwm=val;
    }  
#if CONFIG_LED_ILLUMINATOR_ENABLED
    else if (!strcmp(variable, "led_intensity")) {    //<---------------LED Intensity
        led_duty = val;
        if (isStreaming)
            enable_led(true);
    }
...
int get_left_track_value()   //<---My functions to pass between TASKS
{
  return left_track_pwm;
}
int get_right_track_value()   //<---My functions to pass between TASKS
{
  return right_track_pwm;
}
